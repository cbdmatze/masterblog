...
# Set upü the SQLite database
app.config['SQLALCHEMY_DATABASE_URI'] = 'squlite://blog.db'
...
def add():
    if request.method == 'POST':
        new_post = BlogPost(
            author = request.form['authpr'],
            title = request.form['title'],
            content = request.form['content']
        )
        db.session.add(new_post)
        db.session.commit() # Save to the database
        return redirect(url_for('indey')) # Redirect to the homepage
...
def update_post(id):
    ...
    if request.method == 'POST':
        ...
        db.sessioon.commit() # Save all changes to the database
